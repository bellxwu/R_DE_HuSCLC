...
# Changelog:
# 2025.09.29 - added 7.0 for analysis of the ver2 candidates to be presented and further validated.
...
# 7.2 for top 100 neg hits in CCLE
candidates_100 = muscka_100$id
CCLE_hits = CCLE |> 
  filter(Gene %in% candidates)
...
d_p = ggplot(data = d_candidate, mapping = aes(x = expression, fill = Gene)) +
  ...
  ggtitle("Density plots of RNA \nexpression in CCLE")
# save plot
ggsave("density_CCLE_RNA_candidates-ver2.png",
       path = "~/R_programming/R_DE_HuSCLC/Analysis_PNGs/",
       plot = d_p,
       dpi = 300,
       height = 3,
...
# 7.3 histogram from George_2015 dataset -------------------------------------------------------------
s_cols = colnames(George_hits)[-1] # take samples 
d_candidate = George_hits |>  # create a df_long
  pivot_longer(cols = s_cols,
               names_to = "samples",
...
# save plot
ggsave("histogram_George_RNA_candidate-ver2.png",
       path = "~/R_programming/R_DE_HuSCLC/Analysis_PNGs/",
       plot = h_p,
       dpi = 300,
       height = 3,
